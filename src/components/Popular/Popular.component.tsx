...
import usePopularMoviesStore from "../../store/popularMoviesStore";

const PopularComponent = () => {
  const [currentPage, setCurrentPage] = useState(1); // State for current page
  const { movies, loading, error, fetchPopularMovies, totalPages } =
    usePopularMoviesStore();

  useEffect(() => {
    fetchPopularMovies(currentPage); // Fetch movies for the current page
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {movies.map((movie) => (
          <Link to={`/popularmovies/${movie.id}`} key={movie.id}>
