...
interface TopRatedMovieState {
  ...
  fetchTopRatedMovies: (page: number) => Promise<void>; // Modify the fetch function to accept a page
}

const useTopRatedMoviesStore = create<TopRatedMovieState>((set) => ({
  movies: [],
  loading: false,
  error: null,
  ...
  fetchTopRatedMovies: async (page = 1) => {
    ...
    try {
      ...
      set({ error: "Failed to load top-rated movies.", loading: false });
    }
  },
}));

export default useTopRatedMoviesStore;
